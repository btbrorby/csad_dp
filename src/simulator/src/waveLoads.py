...
class Wave():
    ...
    def getWaveLoads(self):
        ...
        if (self.regular == False):
            ...
            firstOrderLoads = self.getFirstOrderLoad(elementFrequencies, self.angleWaveBody, elementAmplitude, elementPhase)    #Does it reflect first order loads?
            driftLoads = self.getDriftLoads(elementFrequencies, self.angleWaveBody, elementAmplitude)   #Gives too high values...
            slowlyVaryingLoads = self.getSlowlyVaryingLoads(elementFrequencies, self.angleWaveBody, elementAmplitude, phaseLag=elementPhase) #To high values and not slowly varying...
...
# seastate = Wave(0.1, 2.4, regular=False)
